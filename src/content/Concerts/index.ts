...
import {
  ConcertRoute as fragment,
  ConcertCollectionEntry as query,
  type ConcertCollectionEntryQuery,
  type ConcertRouteFragment,
} from '@lib/datocms/types';
import { defineCollection, z } from 'astro:content';

type Meta<T extends ConcertCollectionEntryQuery['record']> = {
...
const loadEntry = async (path: string) => {
  ...
  const { record } = await datocmsRequest<ConcertCollectionEntryQuery>({
    query,
    variables,
  });
...
const loadCollection = async () => {
  const items = (
    await datocmsCollection<ConcertRouteFragment>({
      collection: name,
      fragment,
    })
  )
