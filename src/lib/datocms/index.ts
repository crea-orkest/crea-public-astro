...
export async function datocmsRequest<
  Query,
  AssertReturnValue extends boolean = true,
>({
  query,
  variables = {},
  retryCount = 1,
}: DatocmsRequest): Promise<
  AssertReturnValue extends true ? Assert<Query> : Query
> {
  ...
  if (errors) {
    ...
    const operation = type && name ? `"${type} ${name}"` : 'unknown operation';
    throw Error(
